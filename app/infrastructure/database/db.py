...
async def add_user(
    ...
    logger.info("User added. Table=`%s`, user_id=%d, created_at='%s', "
...
async def get_user_alive_status(
    ...
    async with conn.cursor() as cursor:
        ...
        row = await data.fetchone()
    if row:
        logger.info("The user with 'user_id'=%s has the alive_status is %s", user_id, row[0])
    else:
        logger.warning("No user with `user_id`=%s found in the database", user_id)
    return row[0] if row else None
...
async def get_user_banned_status_by_id(
    ...
    async with conn.cursor() as cursor:
        ...
        row = await data.fetchone()
    if row:
        logger.info("The user with 'user_id'=%s has the banned_status is %s", user_id, row[0])
    else:
        logger.warning("No user with `user_id`=%s found in the database", user_id)
    return row[0] if row else None
...
async def get_user_banned_status_by_username(
    ...
    async with conn.cursor() as cursor:
        ...
        row = await data.fetchone()
    if row:
        logger.info("The user with 'userdname'=%s has the banned_status is %s", username, row[0])
    else:
